...
character_x_pos = (screen_width / 2) - (character_width / 2)  # 캐릭터 초기 x 위치
character_y_pos = screen_height - character_height - stage_height  # 캐릭터 초기 y 위치
...
# 공 크기에 따른 최초 스피드
# -로 한 것은 공이 튕겨져서 올라갈때 값을 -하면서 올라가야 하기 때문
...
while running:
    ...
    # 무기 위치 조정
    weapons = [[w[0], w[1] - weapon_speed]  # w의 0번째 index는 weapon_x_pos, 1번째 index는 weapon_y_pos
               for w in weapons]  # 결과 for 반복문, 무기 위치를 위로

    # 천장에 닿은 무기 없애기
    # 0보다 큰(아직 천장에 닿지 않은) 무기들만 리스트에 담아서 화면에 표시하는 것이다
    weapons = [[w[0], w[1]] for w in weapons if w[1] > 0]

    # 공 위치 정의
    for ball_idx, ball_val in enumerate(balls):
        ball_pos_x = ball_val["pos_x"]  # 처음 시작 하는 공의 위치를 나타낸 것이다
        ...
        # 세로 위치
        # 스테이지에 튕겨서 올라가는 처리
        if ball_pos_y >= screen_height - stage_height - ball_height:  # 공이 x축에 닿을 때
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        # weapon의 for이 ball의 for에 종속되어 있는 이유는 캐릭터가 공을 맞으면 바로 끝나기 때문, 맞지 않았을 때 코드를 돌리기 위해 ball의 for문에 종속시켰다
        # 공과 무기들 충돌 처리
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                if ball_img_idx < 3:
                    ...
                    balls.append({
                        ...
                        "img_idx": ball_img_idx + 1,  # 공의 image index
                        "to_x": -3,  # 공의 x축 이동 방향, -3이면 왼쪽 +3이면 오른쪽
                        "to_y": -6,  # 공의 y축 이동 방향
